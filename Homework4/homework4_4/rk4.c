...
void runge_kutta_4_update(ODE ode,double h,double x){
    ...
    (ode->function)(derivates_eval,ode->k_3,x);

    //Update k_4
    for(int i = 0; i < ode->q ; i++){
        derivates_eval[i] = ode->q_n[i]+h*ode->k_3[i];
    }
    (ode->function)(derivates_eval,ode->k_4,x);
...
void set_func(ODE ode, void (*func)(double*,double*,double)){
     ode->function = func;
}
